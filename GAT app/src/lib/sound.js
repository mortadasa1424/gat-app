...
export const Sound = {
  ...
  correct() { if (!enabled) return; const c = ac(); if (!c) return; const t = c.currentTime; tone(659, t, 0.12, { type: "sine", vol: 0.14 }); tone(880, t + 0.08, 0.2, { type: "sine", vol: 0.14 }); },
  wrong() { if (!enabled) return; const c = ac(); if (!c) return; const t = c.currentTime; tone(196, t, 0.18, { type: "sawtooth", vol: 0.1, glideTo: 150 }); },
  tick() { if (!enabled) return; const c = ac(); if (!c) return; tone(440, c.currentTime, 0.03, { type: "square", vol: 0.05 }); },
